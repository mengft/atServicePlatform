import * as TrafficProvidersInformationServices from '../../services/TrafficServiceProviders/TrafficProduct'
...
export default {
  ...
  effects: {
    *fetch( { payload: { current,car_type,province_id,city_id,service_company,isTraffic } }, {call, put}){
      if(isTraffic != 0 || isTraffic != "0" ){
        isTraffic=1;
      }
      const data = yield call(TrafficProvidersInformationServices.fetchTrafficInformation, { current:current,car_type:car_type,province_id:province_id,city_id:city_id,service_company:service_company,isTraffic:isTraffic });
      if (data.code == 'SUCCESS') {
        ...
        const result = yield call(TrafficProvidersInformationServices.fetchTrafficTypeInformation, {});
        if(result.code == 'SUCCESS'){
          let cascaderOptions = result.message;
    ...
    *handleTrafficProductInformation( { payload: object }, {call, put}) {
      if(object.type == 'add' || object.type === 'edit'){
         const data = yield call(TrafficProvidersInformationServices.addTrafficProduct, object);
         if (data.code == 'SUCCESS') {
            if (object.type == 'add') {
              success('数据添加成功!');
