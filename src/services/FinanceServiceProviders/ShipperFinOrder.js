...
export function fetchInformation({current,car_type,province_id,city_id,status}) {
  ...
  return request(`/api/finance_shipper/order/list?`+search, {headers: newHeaers()});
}
...
export function delOrder({ id}) {
	return request(`/api/finance_shipper/order/delete?productId=${id}`, {headers: newHeaers()});
...
export function handleOrder({ id}) {
	return request(`/api/finance_shipper/order/handle?productId=${id}`, {headers: newHeaers()});
